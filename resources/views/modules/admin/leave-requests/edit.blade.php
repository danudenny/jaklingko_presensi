...
<div class="container mx-auto py-6">
    <x-page-title>
        <x-slot name="title">Edit Pengajuan Cuti</x-slot>
        <x-slot name="actions">
        ...
        <form action="{{ route('leave-requests.update', $leaveRequest) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="driver_id" name="driver_id" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md @error('driver_id') border-red-500 @enderror">
                        ...
                        @foreach($drivers as $driver)
                            <option value="{{ $driver->id }}" {{ (old('driver_id', $leaveRequest->driver_id) == $driver->id) ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <div class="mt-1 relative rounded-md shadow-sm">
                                <input type="text" id="date-range" name="date_range" placeholder="Pilih tanggal"
                                    ...
                                    value="{{ old('start_date', $leaveRequest->start_date->format('Y-m-d')) }} to {{ old('end_date', $leaveRequest->end_date->format('Y-m-d')) }}" readonly>
                                <input type="hidden" id="start_date" name="start_date" value="{{ old('start_date', $leaveRequest->start_date->format('Y-m-d')) }}">
                                <input type="hidden" id="end_date" name="end_date" value="{{ old('end_date', $leaveRequest->end_date->format('Y-m-d')) }}">
                            ...
                            <div class="date-preview mt-2">
                                <div>
                                    ...
                                    <span id="start-date-display" class="ml-1 text-sm font-medium">{{ $leaveRequest->start_date->format('d M Y') }}</span>
                                </div>
                                <div class="mx-2 text-gray-400">→</div>
                                <div>
                                    <span class="text-sm text-gray-500">Sampai:</span>
                                    <span id="end-date-display" class="ml-1 text-sm font-medium">{{ $leaveRequest->end_date->format('d M Y') }}</span>
                                </div>
                                <div class="ml-auto">
                                    <span class="days-count">
                                        <i class="fas fa-calendar-day mr-1"></i>
                                        <span id="days-count">{{ $leaveRequest->start_date->diffInDays($leaveRequest->end_date) + 1 }}</span> hari
                                    </span>
                                </div>
                            </div>
                        ...
                        <div>
                            ...
                            <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md">
                                <div class="space-y-1 text-center">
                                    <div id="preview-container" class="{{ $leaveRequest->documentation ? '' : 'hidden' }} mb-3">
                                        <img id="image-preview" class="mx-auto h-32 object-cover rounded" src="{{ $leaveRequest->documentation ? asset('storage/' . $leaveRequest->documentation) : '#' }}" alt="Preview">
                                    </div>
                ...
                <div>
                    ...
                    <select id="status" name="status" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md @error('status') border-red-500 @enderror">
                        <option value="requested" {{ old('status', $leaveRequest->status) == 'requested' ? 'selected' : '' }}>Menunggu Persetujuan</option>
                        <option value="approved" {{ old('status', $leaveRequest->status) == 'approved' ? 'selected' : '' }}>Disetujui</option>
                        <option value="rejected" {{ old('status', $leaveRequest->status) == 'rejected' ? 'selected' : '' }}>Ditolak</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <textarea id="reason" name="reason" rows="3" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md @error('reason') border-red-500 @enderror">{{ old('reason', $leaveRequest->reason) }}</textarea>
                    @error('reason')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea id="admin_notes" name="admin_notes" rows="3" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md @error('admin_notes') border-red-500 @enderror">{{ old('admin_notes', $leaveRequest->admin_notes) }}</textarea>
                    @error('admin_notes')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        flatpickr("#inline-calendar", {
            ...
            onChange: function(selectedDates, dateStr, instance) {
                if (selectedDates.length === 2) {
                    // Update the main date range picker
                    dateRangePicker.setDate([selectedDates[0], selectedDates[1]]);
                }
            }
        });
