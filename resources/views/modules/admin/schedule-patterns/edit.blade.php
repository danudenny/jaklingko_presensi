...
<div class="w-full px-4 container-fluid">
    <x-page-title>
        <x-slot name="title">
            <div class="flex items-center">
                ...
                <div>
                    <h1 class="text-2xl font-bold">Edit Pola Jadwal</h1>
                    <p class="text-sm font-thin text-gray-500">Edit pola jadwal yang ada</p>
                </div>
            </div>
        </x-slot>
        <x-slot name="actions">
    ...
    <div class="p-4 bg-white rounded-lg shadow">
        <form action="{{ route('schedule-patterns.update', $pattern->id) }}" method="POST" id="patternForm">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                <div>
                    <div class="mb-4">
                        ...
                        <input type="text" name="name" id="name" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" value="{{ old('name', $pattern->name) }}" required>
                        @error('name')
                    ...
                    <div class="mb-4">
                        ...
                        <select name="type" id="type" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                            <option value="valid" {{ old('type', $pattern->type) == 'valid' ? 'selected' : '' }}>Valid</option>
                            <option value="invalid" {{ old('type', $pattern->type) == 'invalid' ? 'selected' : '' }}>Invalid</option>
                        </select>
                        @error('type')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="mb-4">
                        ...
                        <select name="driver_type" id="driver_type" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                            <option value="all" {{ old('driver_type', $pattern->driver_type) == 'all' ? 'selected' : '' }}>Semua</option>
                            <option value="batangan" {{ old('driver_type', $pattern->driver_type) == 'batangan' ? 'selected' : '' }}>Batangan</option>
                            <option value="cadangan" {{ old('driver_type', $pattern->driver_type) == 'cadangan' ? 'selected' : '' }}>Cadangan</option>
                        </select>
                        @error('driver_type')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="mb-4">
                        ...
                        <select name="days" id="days" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                            @for ($i = 1; $i <= 15; $i++)
                                <option value="{{ $i }}" {{ old('days', $pattern->days) == $i ? 'selected' : '' }}>{{ $i }} hari</option>
                            @endfor
                        </select>
                        @error('days')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="description" id="description" rows="3" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ old('description', $pattern->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        <div class="flex items-center">
                            <input type="checkbox" name="is_active" id="is_active" class="w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500" value="1" {{ old('is_active', $pattern->is_active) ? 'checked' : '' }}>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Get existing pattern from PHP
        const existingPattern = @json($pattern->pattern);
        
        // Initial pattern generation
        generatePattern(daysSelect.value, existingPattern);
        
        // Update pattern when days change
        daysSelect.addEventListener('change', function() {
            generatePattern(this.value, existingPattern);
        });
        
        function generatePattern(days, existingPattern) {
            ...
            for (let i = 0; i < days; i++) {
                ...
                // Use existing pattern if available, otherwise default to Pagi for first cell
                const shift = (existingPattern && i < existingPattern.length) ? existingPattern[i] : (i === 0 ? 'P' : 'N');
                
                // Create cell
                const cell = document.createElement('div');
                cell.className = `pattern-cell pattern-${shift === 'P' ? 'pagi' : (shift === 'S' ? 'siang' : 'none')} selected`;
                cell.dataset.shift = shift;
                cell.textContent = shift === 'N' ? '-' : shift;
