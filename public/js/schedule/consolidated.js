...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (showLegendsBtn) {
        showLegendsBtn.addEventListener("click", () =>
            openDrawer(legendsDrawer, legendsOverlay)
        );
    }

    if (closeLegendsBtn) {
        closeLegendsBtn.addEventListener("click", () =>
            closeDrawer(legendsDrawer, legendsOverlay)
        );
    }

    if (legendsOverlay) {
        legendsOverlay.addEventListener("click", () =>
            closeDrawer(legendsDrawer, legendsOverlay)
        );
    }
    ...
    const unassignedDrawer = document.getElementById(
    ...
    if (showUnassignedBtn) {
        showUnassignedBtn.addEventListener("click", () =>
            openDrawer(unassignedDrawer, unassignedOverlay)
        );
    }

    if (closeUnassignedBtn) {
        closeUnassignedBtn.addEventListener("click", () =>
            closeDrawer(unassignedDrawer, unassignedOverlay)
        );
    }

    if (unassignedOverlay) {
        unassignedOverlay.addEventListener("click", () =>
            closeDrawer(unassignedDrawer, unassignedOverlay)
        );
    }
    ...
    if (showStatsBtn) {
        showStatsBtn.addEventListener("click", () =>
            openDrawer(statsDrawer, statsOverlay)
        );
    }

    if (closeStatsBtn) {
        closeStatsBtn.addEventListener("click", () =>
            closeDrawer(statsDrawer, statsOverlay)
        );
    }
    ...
    const unassignedFilterType = document.getElementById(
        "unassigned-filter-type"
    );
    if (unassignedFilterType) {
        unassignedFilterType.addEventListener("change", function () {
            const selectedType = this.value;
            const batanganSection = document
                .getElementById("unassigned-batangan-list")
                .closest("div.mb-5");
            const cadanganSection = document
                .getElementById("unassigned-cadangan-list")
                .closest("div.mb-5");
    ...
    document.addEventListener("keydown", function (e) {
        if (
            e.key === "Escape" &&
            !legendsDrawer.classList.contains("translate-x-full")
        ) {
            closeLegendsDrawer();
        }
    });
    ...
    document.querySelectorAll(".unit-header").forEach((header) => {
        ...
        if (actionsContainer) {
            ...
            editButton.className =
                "edit-unit-btn ml-2 text-xs bg-orange-100 text-orange-700 hover:bg-orange-200 px-2 py-1 rounded-md";
            editButton.innerHTML = '<i class="fas fa-edit mr-1"></i> Edit';
            editButton.setAttribute("data-unit-id", unitId);

            editButton.addEventListener("click", function (e) {
                e.stopPropagation();
                toggleUnitEditMode(unitId);
            });
                    ...
                    document
                        ...
                        .addEventListener("click", function () {
                            ...
                            checkedBoxes.forEach((checkbox) => {
                                const unitId =
                                    checkbox.getAttribute("data-unit-id");
                                const date = checkbox.getAttribute("data-date");
                                const shift =
                                    checkbox.getAttribute("data-shift");
    ...
    function findUnitRow(row) {
        ...
        while (current) {
            ...
            if (
                current.classList &&
                (current.classList.contains("unit-header") ||
                    current.classList.contains("bg-blue-50"))
            ) {
                return current;
            }
    ...
    function getShiftFromRow(row) {
        ...
        return row.classList && row.classList.contains("morning-shift")
            ? "pagi"
            : "siang";
    }
    ...
    function checkIfUnitInRenops(unitId, dateValue) {
        ...
        for (const indicator of renopsIndicators) {
            ...
            if (indicatorUnitId === unitId) {
                ...
                const dateCell = table.querySelector(
    ...
    function checkIfUnitInMaintenance(unitId, dateValue) {
        ...
        const maintenanceIndicators =
            document.querySelectorAll(".text-teal-800");
        if (!maintenanceIndicators || maintenanceIndicators.length === 0)
            return false;
    ...
    // Unit-specific edit mode functionality
    let unitEditModes = {};

    function toggleUnitEditMode(unitId) {
        // Toggle the edit mode for this specific unit
        unitEditModes[unitId] = !unitEditModes[unitId];

        const unitContent = document.querySelector(
            `.unit-content[data-unit-content="${unitId}"]`
        );
        const editButton = document.querySelector(
            `.edit-unit-btn[data-unit-id="${unitId}"]`
        );

        if (unitEditModes[unitId]) {
            // Enter edit mode for this unit
            if (editButton) {
            ...
            // Convert cells to checkboxes only for this unit
            convertUnitToCheckboxes(unitId);

            // Add save button for this unit
            addSaveButtonForUnit(unitId);
        } else {
            // Exit edit mode for this unit
            if (editButton) {
                editButton.innerHTML = '<i class="fas fa-edit mr-1"></i> Edit';
                editButton.classList.add(
                    "bg-orange-100",
            ...
            // Convert back to normal view only for this unit
            convertUnitToNormalView(unitId);

            // Remove save button for this unit
            removeSaveButtonForUnit(unitId);
        }
    }

    function convertUnitToCheckboxes(unitId) {
        // Get all date cells in this unit
        const unitContent = document.querySelector(
            `.unit-content[data-unit-content="${unitId}"]`
        );
        if (!unitContent) return;

        const dateCells = unitContent.querySelectorAll(
        ...
        dateCells.forEach((parentTd) => {
            ...
            // Get driver ID from the row
            const driverLink = row.querySelector("td:nth-child(3) a");
            let driverId = null;
            if (driverLink) {
                const href = driverLink.getAttribute("href");
                if (href) {
                    const matches = href.match(/\/drivers\/([0-9]+)/);
            ...
            const isUnitInMaintenance = checkIfUnitInMaintenance(
                unitId,
                dateValue
            );
            ...
            const hasBackupIndicator =
                parentTd.querySelector(".text-amber-800");
            const hasRenopsIndicator =
                parentTd.querySelector(".renops-indicator");
            ...
            if (
                ...
                checkbox.setAttribute("data-driver-id", driverId);
            ...
            const checkbox = document.createElement("input");
            checkbox.type = "checkbox";
            const isChecked = hasAssignedIndicator || hasBackupIndicator;
            checkbox.checked = isChecked;
            checkbox.setAttribute("data-originally-checked", isChecked ? "true" : "false");
    ...
    function convertUnitToNormalView(unitId) {
        const unitContent = document.querySelector(
            `.unit-content[data-unit-content="${unitId}"]`
        );
        if (!unitContent) return;

        const checkboxCells = unitContent.querySelectorAll(
            'td input[type="checkbox"]'
        );
    ...
    function addSaveButtonForUnit(unitId) {
        const unitContent = document.querySelector(
            `.unit-content[data-unit-content="${unitId}"]`
        );
        if (!unitContent) return;
        ...
        saveRow.className = "unit-save-row";
        saveRow.setAttribute("data-unit-save", unitId);
        saveRow.innerHTML = `
            <td colspan="100%" class="px-4 py-3 bg-gray-100">
                <div class="flex justify-end">
                    <button class="unit-save-btn inline-flex items-center px-4 py-2 text-xs font-semibold tracking-widest text-white uppercase transition duration-150 ease-in-out border border-transparent rounded-md shadow bg-gradient-to-r from-green-600 to-green-700 hover:from-green-500 hover:to-green-600">
                        <i class="mr-2 fas fa-save"></i>
                        Simpan Perubahan
                    </button>
                </div>
        ...
        // Add the save button after the unit content
        unitContent.appendChild(saveRow);

        // Add event listener to the save button
        const saveButton = saveRow.querySelector(".unit-save-btn");
        saveButton.addEventListener("click", function () {
            saveUnitChanges(unitId);
        });
    }

    function removeSaveButtonForUnit(unitId) {
        const saveRow = document.querySelector(
            `tr[data-unit-save="${unitId}"]`
        );
        if (saveRow && saveRow.parentNode) {
            saveRow.parentNode.removeChild(saveRow);
        }
    }
});
